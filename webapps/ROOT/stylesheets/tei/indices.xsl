...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="result/doc">
    <tr>
      ...
      <xsl:apply-templates select="arr[@name='lingua']"/>
      <xsl:apply-templates select="arr[@name='index_instance_location']" />
      <xsl:apply-templates select="str[@name='persone_menzionate']" />
      <xsl:apply-templates select="str[@name='luoghi_menzionati']" />
      <xsl:apply-templates select="str[@name='istituzioni_menzionate']" />
      <xsl:apply-templates select="str[@name='autori_menzionati']" />
      <xsl:apply-templates select="str[@name='opere_menzionate']" />
      <xsl:apply-templates select="str[@name='fonti_menzionate']" />
      <xsl:apply-templates select="str[@name='topic']" />
    </tr>
  </xsl:template>
  ...
  <xsl:template match="arr[@name='lingua']">
    <td>
  ...
  <xsl:template match="arr[@name='lingua']/str">
    <li>
      <xsl:value-of select="."/>
    </li>
  </xsl:template>
  
  <xsl:template match="str[@name='person']">
    <td>
      <xsl:value-of select="." />
    </td>
  </xsl:template>

  <xsl:template match="str[@name='place']">
    <td>
      <xsl:value-of select="." />
    </td>
  </xsl:template>
  
  <xsl:template match="str[@name='institution']">
    <td>
      <xsl:value-of select="." />
    </td>
  </xsl:template>
  
  <xsl:template match="str[@name='autori_menzionati']">
    <td>
      <xsl:value-of select="." />
    </td>
  </xsl:template>
  
  <xsl:template match="str[@name='work']">
    <td>
      <xsl:value-of select="." />
    </td>
  </xsl:template>
  
  <xsl:template match="str[@name='source']">
    <td>
      <xsl:value-of select="." />
    </td>
  </xsl:template>
  
  <xsl:template match="str[@name='tematiche']">
    <td>
      <xsl:value-of select="." />
    </td>
  </xsl:template>
