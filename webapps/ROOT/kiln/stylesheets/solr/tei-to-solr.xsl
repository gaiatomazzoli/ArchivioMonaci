...
<xsl:stylesheet exclude-result-prefixes="#all"
  ...
  <xsl:template match="tei:repository" mode="facet_collocazione_attuale">
    <field name="collocazione_attuale">
      <xsl:choose>
        <!--<xsl:when test="@ref"><xsl:value-of select="@ref"/></xsl:when>-->
        <xsl:when test="contains(.,'Archivio Monaci')"><xsl:text>Archivio Monaci, Sapienza Università di Roma</xsl:text></xsl:when>
        <xsl:when test="contains(.,'Marucelliana')"><xsl:text>Archivio Rajna, Biblioteca Marucelliana di Firenze</xsl:text></xsl:when>
        <xsl:when test="contains(.,'Ascoli')"><xsl:text>Archivio Ascoli, Biblioteca dell’Accademia Nazionale dei Lincei e Corsiniana di Roma</xsl:text></xsl:when>
        <xsl:when test="contains(.,'Bibliothèque Nationale de France')"><xsl:text>Bibliothèque Nationale de France</xsl:text></xsl:when>
        <xsl:when test="contains(.,'Società geografica italiana')"><xsl:text>Società Geografica Italiana</xsl:text></xsl:when>
        <xsl:when test="contains(.,'Fondo Parodi')"><xsl:text>Fondo Parodi, Università di Firenze</xsl:text></xsl:when>
        <xsl:when test="contains(.,'Fondo D’Ancona')"><xsl:text>Fondo D’Ancona, Scuola Normale Superiore di Pisa</xsl:text></xsl:when>
        <xsl:when test="contains(.,'Carte Novati')"><xsl:text>Carte Novati, Biblioteca Braidense di Milano</xsl:text></xsl:when>
        <xsl:when test="contains(.,'Fondo Rossi')"><xsl:text>Fondo Rossi, Biblioteca Alessandrina di Roma</xsl:text></xsl:when>
  ...
  <xsl:template name="field_collocazione_attuale">
    <xsl:apply-templates mode="facet_collocazione_attuale" select="//tei:teiHeader/tei:fileDesc/tei:sourceDesc//tei:repository"/>
  </xsl:template>
